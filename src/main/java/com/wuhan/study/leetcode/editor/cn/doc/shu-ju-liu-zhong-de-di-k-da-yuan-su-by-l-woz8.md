...
在单次插入的操作中，我们首先将元素 ![\textit{val} ](./p__textit{val}_.png)  加入到优先队列中。如果此时优先队列的大小大于 *k*，我们需要将优先队列的队头元素弹出，以保证优先队列的大小为 *k*。
...
- 时间复杂度：
  ...
  - 初始化时间复杂度为：![O(n\logk) ](./p__O_n_log_k__.png)  ，其中 *n* 为初始化时 ![\textit{nums} ](./p__textit{nums}_.png)  的长度；

  - 单次插入时间复杂度为：![O(\logk) ](./p__O_log_k__.png) 。
