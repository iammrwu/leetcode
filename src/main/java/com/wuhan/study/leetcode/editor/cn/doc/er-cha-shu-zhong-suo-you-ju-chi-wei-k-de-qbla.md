...
若将 ![\textit{target} ](./p__textit{target}_.png)  当作树的根结点，我们就能从 ![\textit{target} ](./p__textit{target}_.png)  出发，使用深度优先搜索去寻找与 ![\textit{target} ](./p__textit{target}_.png)  距离为 *k* 的所有结点，即深度为 *k* 的所有结点。

由于输入的二叉树没有记录父结点，为此，我们从根结点 ![\textit{root} ](./p__textit{root}_.png)  出发，使用深度优先搜索遍历整棵树，同时用一个哈希表记录每个结点的父结点。

然后从 ![\textit{target} ](./p__textit{target}_.png)  出发，使用深度优先搜索遍历整棵树，除了搜索左右儿子外，还可以顺着父结点向上搜索。

代码实现时，由于每个结点值都是唯一的，哈希表的键可以用结点值代替。此外，为避免在深度优先搜索时重复访问结点，递归时额外传入来源结点 ![\textit{from} ](./p__textit{from}_.png) ，在递归前比较目标结点是否与来源结点相同，不同的情况下才进行递归。
