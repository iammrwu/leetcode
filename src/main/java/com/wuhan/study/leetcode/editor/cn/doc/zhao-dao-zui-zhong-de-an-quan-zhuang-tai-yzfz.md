...
具体来说，首先得到反图 ![\textit{rg} ](./p__textit{rg}_.png)  及其入度数组 ![\textit{inDeg} ](./p__textit{inDeg}_.png) 。将所有入度为 *0* 的点加入队列，然后不断取出队首元素，将其出边相连的点的入度减一，若该点入度减一后为 *0*，则将该点加入队列，如此循环直至队列为空。循环结束后，所有入度为 *0* 的节点均为安全的。我们遍历入度数组，并将入度为 *0* 的点加入答案列表。
